...
def filter_routes_by_city(routes):
    return [route for route in routes if route[3] in ATLANTIC_CITIES]
...
if __name__ == "__main__":
    ...
    if atlantic_routes:
        ...
        routes_to_process = [(r[0], r[1], r[2]) for r in atlantic_routes]
        print(f"Found {len(routes_to_process)} routes for the Atlantic time zone.")
        get_commute_routes(routes_to_process, project_id, bucket_name, output_filename_prefix="commute_routes_atlantic")
    else:

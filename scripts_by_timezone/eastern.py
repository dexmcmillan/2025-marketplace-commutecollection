# Eastern Time Zone: Ottawa, Toronto
import pandas as pd
import sys
import os
...
def filter_routes_by_city(routes):
    return [route for route in routes if route[3] in EASTERN_CITIES]
...
if __name__ == "__main__":
    ...
    eastern_routes = filter_routes_by_city(all_routes)
    ...
    if eastern_routes:
        ...
        routes_to_process = [(r[0], r[1], r[2]) for r in eastern_routes]
        print(f"Found {len(routes_to_process)} routes for the Eastern time zone.")
        get_commute_routes(routes_to_process, project_id, bucket_name, output_filename_prefix="commute_routes_eastern")
    else:
        print("No routes found for the Eastern time zone.")

...
def filter_routes_by_city(routes):
    return [route for route in routes if any(city in route[0] for city in EASTERN_CITIES)]
...
if __name__ == "__main__":
    ...
    if eastern_routes:
        print(f"Found {len(eastern_routes)} routes for the Eastern time zone.")
        get_commute_routes(eastern_routes, project_id, bucket_name, output_filename_prefix="commute_routes_eastern")
    else:

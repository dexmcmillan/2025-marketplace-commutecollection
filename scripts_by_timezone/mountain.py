...
def filter_routes_by_city(routes):
    return [route for route in routes if any(city in route[0] for city in MOUNTAIN_CITIES)]
...
if __name__ == "__main__":
    ...
    if mountain_routes:
        print(f"Found {len(mountain_routes)} routes for the Mountain time zone.")
        get_commute_routes(mountain_routes, project_id, bucket_name, output_filename_prefix="commute_routes_mountain")
    else:
